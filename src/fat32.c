...
#else
                        // copy basename
                        copy_from_ram(loc, _basename, 8);
#endif

                        if(attrPos & 0x10) { // directory entry
#ifdef LONG_FILE_NAMES
                            sprintf(termbuffer, "%c%3u%c%-24.24s%c (dir)", COL_YELLOW, fctr, COL_WHITE, _filename, COL_CYAN);
#else
                            sprintf(termbuffer, "%c%3u%c%.8s DIR       %c%08lX", COL_YELLOW, fctr, COL_WHITE, _basename, COL_CYAN, fc);
#endif
                        } else {             // file entry
...
#ifdef LONG_FILE_NAMES
                                sprintf(termbuffer, "%c%3u%c%-24.24s%c%6lu", COL_GREEN, fctr, COL_WHITE, _filename, COL_YELLOW, filesize);
#else
                                sprintf(termbuffer, "%c%3u%c%.8s.%.3s%c%6lu%c%08lX", COL_GREEN, fctr, COL_WHITE, _basename, _ext, COL_YELLOW, filesize, COL_CYAN, fc);
#endif
                            }
                        }
                        terminal_printtermbuffer();
